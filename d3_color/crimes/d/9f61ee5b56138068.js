// https://observablehq.com/@castelojb/homicidios-em-fortaleza-em-2012@108
  ...
  main.variable(observer("geojson")).define("geojson", ["L","info","map","bairros","style"], function(L,info,map,bairros,style)
{
  ...
  main.variable(observer("legend")).define("legend", ["L","vermelhos","colorScale","d3","map"], function(L,vermelhos,colorScale,d3,map)
{
  let legendControl = L.control({position: 'bottomright'});
	...
	legendControl.onAdd = function (map) {
		...
		for (let i = 0; i < n; i++) {
			...
			labels.push(
				'<i style="background:' + vermelhos[i] + '"></i> ' +
...
d3.schemeBlues[5]
  ...
  main.variable(observer("vermelhos")).define("vermelhos", ["d3"], function(d3){return(
d3.schemeReds[7]
)});
  main.variable(observer("colorScale")).define("colorScale", ["d3","dataset","vermelhos"], function(d3,dataset,vermelhos){return(
d3.scaleQuantize()
    .domain([0, d3.max(dataset, d => (d.Homicidios * 100000) / d.Populacao)])
    .range(vermelhos)
)});
  main.variable(observer("dataset")).define("dataset", ["d3"], function(d3){return(
d3.csv('https://gist.githubusercontent.com/emanueles/8ee225792454a78151d0842a10642a29/raw/9984e32effa99da3cea2ec87a7c9b7b231c71d3c/fortaleza_crimes_populacao.csv')
)});
  ...
  // control that shows state info on hover
	...
	infoControl.update = function (feat) {
			this._div.innerHTML = '<h5>Taxa de homicídios</h5>' +  (feat ?
				'<b>' + feat.properties.NOME + '</b><br />' + homicidesByName.get(feat.properties.NOME)
...
d3.csv("https://gist.githubusercontent.com/emanueles/8ee225792454a78151d0842a10642a29/raw/9984e32effa99da3cea2ec87a7c9b7b231c71d3c/fortaleza_crimes_populacao.csv").then(function(data) {
    ...
    data.forEach(function(d) {
       bairrosMap.set(d.Bairro,(d.Homicidios * 100000) / d.Populacao)
        })
    return bairrosMap
  })
)});
