// https://observablehq.com/@castelojb/taxa-de-desemprego-nos-eua-em-agosto-de-2016@104
  ...
  main.variable(observer("buildvis")).define("buildvis", ["d3","DOM","topojson","us","rateById"], function(d3,DOM,topojson,us,rateById)
{
  ...
  let path = d3.geoPath()
  let verdes  = d3.scaleSequential(d3.interpolateGreens).domain([1, 10]);
  ...
  svg.append("g")
    ...
    .enter()
    .append("path")
      .attr("fill", d => verdes(rateById['$'+d.id]))
      .attr("d", path)
  .append("title")
    .text(d => 'Porcentagem de desemprego : ' + rateById['$'+d.id])
...
d3.tsv("https://gist.githubusercontent.com/emanueles/7fbdcd8ee412e3e63904114cf9d7ee3a/raw/af4fd93db1be3f9033caff46d4a574a54bf9363f/unemployment.tsv").then(function (data) {
  ...
  main.variable(observer("verdes")).define("verdes", ["d3"], function(d3){return(
d3.scaleOrdinal(d3.schemeGreens[9])
)});
  main.variable(observer()).define(["verdes","rateById"], function(verdes,rateById){return(
verdes(rateById['$01039'])
)});
